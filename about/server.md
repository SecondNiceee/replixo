...
- **`joinRoom`** — проверяет: при отсутствии `create` комната должна
  ...
  участника, уведомляет остальных `peerJoined`, возвращает RTP-capabilities,
...
- **`disconnect`** — неявный обрыв. Участник НЕ удаляется сразу: его
  producers/consumers живут в течение **grace-window 45 с**
  (`DISCONNECT_GRACE_MS`), чтобы блокировка телефона или смена сети не
  «выкидывала» человека. Удаление происходит только если он не вернулся.
