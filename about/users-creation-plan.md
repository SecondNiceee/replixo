> ⚠️ **Исторический план, НЕ соответствует текущему коду.** Этот документ описывает
> первоначальный замысел на **Auth.js v5 (next-auth) + bcryptjs + страницы
> `/login` и `/register`**. В реальности этап был реализован иначе — на
> **Better Auth** (плагин `username`, страницы `/sign-in` и `/sign-up`, без
> ручного bcrypt). Актуальное описание — в
> [`users/authorization.md`](./users/authorization.md). Файл оставлен для истории.
