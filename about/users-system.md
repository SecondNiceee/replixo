> ⚠️ **Исторический план, местами расходится с текущим кодом.** Актуальное
> описание системы друзей (реальные эндпоинты, включая `/api/friends/sent` и
> `/api/friends/cancel`, и реальный UI со вкладками) — в
> [`users/friends.md`](./users/friends.md). Отличия этого плана от факта: поиск
> идёт по **точному username**, а не по `name`/`email`; профиль устроен на
> вкладках «Друзья»/«Мои заявки»; чужой профиль `/profile/[username]` пока не
> реализован. Файл оставлен для истории.
