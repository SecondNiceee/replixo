...
type Tab = "chat" | "annotation" | "sounds"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex gap-1 rounded-lg bg-muted p-1">
          {([['chat', 'Кнопка чата'], ['annotation', 'Вкл/выкл перо'], ['sounds', 'Звуки']] as const).map(([value, label]) => (
            <button key={value} type="button" onClick={() => setTab(value)} className={cn("flex-1 rounded-md px-2 py-1.5 text-sm font-medium transition-colors", tab === value ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>{label}</button>
          ))}
        </div>
        ...
        {tab === "annotation" && <div className="flex flex-col gap-5 py-1">
          <div className="flex flex-col gap-2"><span className="text-sm font-medium">Способ включения</span><div className="flex gap-2"><Button variant={annotation.activation === "double-click" ? "default" : "outline"} size="sm" onClick={() => annotation.setActivation("double-click")}>Двойное нажатие</Button><Button variant={annotation.activation === "hotkey" ? "default" : "outline"} size="sm" onClick={() => annotation.setActivation("hotkey")}>Клавиша</Button></div><span className="text-xs text-muted-foreground">Включает или выключает перо во время демонстрации экрана.</span></div>
        ...
        <div className="-mx-4 -mb-4 flex justify-between gap-2 rounded-b-xl border-t bg-muted/50 p-4"><Button variant="ghost" size="sm" onClick={() => tab === "chat" ? chat.reset() : tab === "annotation" ? annotation.reset() : sounds.reset()}>Сбросить по умолчанию</Button><Button variant="outline" size="sm" onClick={() => onOpenChange(false)}>Готово</Button></div>
      </DialogContent>
    </Dialog>
  )
}
