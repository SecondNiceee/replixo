...
exports.friendIdsOf = friendIdsOf;
...
function statusOf(userId) {
    ...
    for (const presence of sockets.values()) {
        if (presence.status === 'online' || presence.status === 'call')
            return 'online';
    }
 ...
 */
async function trackConnect(nsp, socket, userId, initialStatus = 'online') {
    const now = Date.now();
    ...
    if (statusAfter !== statusBefore) {
        for (const friendId of friends) {
            nsp.to((0, namespace_types_1.userRoom)(friendId)).emit('dm:presence', { userId, status: statusAfter });
        }
    }
