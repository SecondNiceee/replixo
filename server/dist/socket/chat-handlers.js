...
function registerChatHandlers(ctx) {
    ...
    const allowChatEvent = (0, helpers_1.createRateLimiter)(5, 2000);
    socket.on('chatMessage', (payload) => {
        // --- Input validation ---
        if (!payload || typeof payload !== 'object')
            return;
        const { roomId: rid, peerId: pid, text, id: clientId, attachment } = payload;
        ...
        const id = typeof clientId === 'string' && clientId.length > 0 && clientId.length <= 64
            ? clientId
        ...
        void (0, db_1.saveMessage)(message);
    ...
    // -----------------------------------------------------------------------
    socket.on('chatRead', (payload) => {
        if (!payload || typeof payload !== 'object')
            return;
        const { roomId: rid, peerId: pid, ts } = payload;
        if (typeof rid !== 'string' || !rid)
            return;
        if (typeof pid !== 'string' || !pid)
            return;
