import { savePresentationDrawing } from '../db'
import type {
...
export function registerPresentationHandlers(ctx: HandlerContext): void {
  const { io, socket } = ctx
  ...
  const allowSlideEvent = createRateLimiter(10, 1000)

  socket.on('presentationSlide', (payload: unknown) => {
    // --- Input validation ---
  ...
  // -----------------------------------------------------------------------
  socket.on('presentationEnded', (payload: unknown) => {
    if (!payload || typeof payload !== 'object') return
  ...
  const allowStrokeEvent = createRateLimiter(300, 1000)

  socket.on('presentationStroke', (payload: unknown) => {
    if (!payload || typeof payload !== 'object') return
