...
export function decodeOriginalName(originalname: string): string {
  ...
  if (!isLatin1Only(originalname)) return originalname
  // Чистый ASCII → перекодировка ничего не изменит, экономим работу.
  // eslint-disable-next-line no-control-regex
  if (!/[\x80-\xff]/.test(originalname)) return originalname
 ...
 */
export function contentDisposition(rawName: unknown): string {
